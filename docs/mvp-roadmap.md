...
- 主控制台默认进入 `Base` 层
- 左右手输入区之间的中央竖条放置连接状态入口和 `Fn Num`、`Base`、`Mouse Nav` 竖向层控件，默认层 `Base` 位于中间，支持上下滑动切层与点击归中
- 旧 `Keyboard`、`Touchpad` 页面与旧 Settings 入口暂时保留在代码中作为回归参考，不再作为默认导航入口
- 后续应把 Lab 默认布局继续迁入统一 schema/runtime，而不是把当前硬编码 Lab 形态当作最终产品模型
