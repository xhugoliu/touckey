...
当前应用默认进入 `Base` 层，左右手输入区之间的中央竖条内放置连接状态入口和 `Fn Num`、`Base`、`Mouse Nav` 竖向层控件，其中默认层 `Base` 位于中间；层控件通过上下滑动切换，点击归中到 `Base`。

旧 `Keyboard`、`Touchpad` 页面和旧 Settings 入口不再作为默认或可见导航入口；它们暂时保留在代码中，用作早期 HID 行为、触控板行为和 schema/runtime 迁移的回归参考。
